...
- Saved `.fdreport` files remember the selected report overview, file, section, and report item where possible when reopened.
...
- Any release that adds new file-type families must include a generated `.fdreport` opened in FileDentify with a representative subset of those new types for manual review before publishing.
...
- Ensoniq EPS/ASR/TS sampler files such as `.efe`, `.eda`, and `.edt` show an Ensoniq sampler section with file role, header title, visible catalog strings, size, and a proprietary-payload note. The contributed `Ensoniq-stuff.zip` sample set is a useful release-check source when available.
...
- Game and ROM files such as `.nsf`, `.nes`, `.rom`, `.gb`, `.gbc`, `.gba`, `.nds`, `.sfc`, `.smc`, `.gen`, `.md`, `.sms`, `.gg`, `.n64`, `.z64`, `.wad`, `.chd`, `.pak`, `.bsp`, `.mdl`, `.spr`, `.dem`, `.qwd`, `progs.dat`, `.bpak`, `.bnk`, `.wem`, `.assets`, `.rpa`, `.acf`, `.vdf`, `.cue`, `.gdi`, `.sav`, `.srm`, `.ips`, and `.bps` show a Game/ROM data section. NSF files should show title, artist, song count, addresses, timing, video system, and expansion audio where present. iNES files should show mapper, PRG/CHR ROM sizes, mirroring, trainer, and battery flags. Nintendo DS files should show title, game code, maker code, unit code, ROM capacity, ARM offsets/sizes, file-name table info, and header CRC. Sega Master System/Game Gear files should show the `TMR SEGA` marker, checksum, product code, version, region/system, and ROM size code. Doom WAD, CHD, Quake BSP/demo/progs/model/sprite, and protected `.bpak` files should expose their family and safe header/path clues. Steam/Unity/Wwise-style files should at least identify the family and expose header fields or manifest keys where present.
...
- PyTorch-style `.ckpt`, `.pt`, and `.pth` checkpoints and SafeTensors `.safetensors` files show an AI model / Ollama section with container type, ZIP entry or pickle hints, SafeTensors declared header size, and a note that FileDentify does not load the model.
- Piper/Sonata/SuperTonic voice files show a Speech voice section for `.onnx`, sidecar `.json`, and `MODEL_CARD` files, including model role, voice folder, sample rate, quality, eSpeak voice, language, and speaker counts where visible.
...
- QWS files such as `qws.ini`, `inst_*.ini`, `notexfrm.ini`, `userxfrm.ini`, and `qws.lng` show a QWS sequencer section. Instrument files should show the map name, patch count, and example patches. Transform files should show transform counts. `qws.ini` should count recent-file entries without printing the actual recent paths.
- NVDA add-ons such as `.nvda-addon` show an NVDA add-on section with manifest name, summary, version, author, minimum NVDA version, last tested NVDA version, documentation file, update channel, entry counts, and top-level package entries.
...
- PC BIOS/UEFI images, Android boot images, U-Boot legacy uImage files, UF2 microcontroller firmware, Broadcom/OpenWrt TRX router firmware, flattened device tree blobs, Intel HEX, Motorola S-records, and MSI-style BIOS filenames such as `E7A32...` show a Firmware / device image section with header marker, file size, useful header/record fields, visible firmware strings where appropriate, and a clear no-flash/no-modify note.
- `usb.ids` and `pci.ids` files show a Hardware ID database section with version/date comments, sampled vendor/device/interface/class counts, and example entries.
- Personal/export formats such as `.vmg`, `.vcf`, `.ics`, `.opml`, `.eml`, and `.wme` show a Message/contact data section with structure counts and selected metadata plus a privacy note. Real OPML should be detected by XML markers, not by extension alone.
- Windows system and app-support files such as `.evtx`, `.pnf`, `.inf_loc`, `.gpd`, `.gdl`, `.xrm-ms`, `.sdb`, `catdb`, `.cat`, `.appx`, `.msix`, `.msixbundle`, `.manifest`, `.mo`, `.qm`, and the System32 `drivers\etc` network config files show a Windows/system data section with safe header, text/XML, and package-structure details. FileDentify must not execute, import, install, register, or modify these files while inspecting them.
- VMware metadata files such as `.vmx`, `.vmsd`, `.vmxf`, `.nvram`, and `.scoreboard` show a Virtual machine metadata section with selected safe VM details. UUIDs, MAC addresses, and full host paths must not be printed in that section.
- Electron `.asar` archives and NuGet `.nupkg` packages show a Developer/app resources section with ASAR index summaries or NuGet nuspec metadata and package counts.
- Acapela `.qvcu` and `.nuul216` speech voice assets show a Speech voice section with voice folder, role, and size.
- Microsoft natural voice packages/data, AT&T Natural Voices packages, IVONA and Loquendo voice packages, Dolphin `voices.ini` / Nuance Vocalizer Expressive / Dolphin Orpheus speech-component files, Eloquence/IBM TTS `.syn` modules, RHVoice voice/model/lexicon files, FlexVoice data, and eSpeak NG dictionaries/phoneme tables show a Speech voice section with vendor/family, role, package payloads, engine manifest details, or voice preference details where available. Large voice archives must be summarized from indexes/headers only; FileDentify must not run installers or extract full voice payloads during normal inspection.
...
- The embedded manual changelog keeps one section each for 1.4, 1.3, 1.2, 1.1.1, 1.1, and 1.0, in that order. New release notes must not overwrite or bleed into previous release sections.
