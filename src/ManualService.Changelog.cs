...
namespace FileDentify
{
    internal static partial class ManualService
    {
        private static void Changelog(StringBuilder html)
        {
            ...
            html.AppendLine("<li>Added Pocket TTS reporting for NVDA speech-engine files, including ONNX model roles, NumPy voice embeddings, tokenizer models, and safe shape/string metadata.</li>");
            html.AppendLine("<li>Expanded NVDA and Python ecosystem reporting from a full NVDA roaming-folder scan, including installed NVDA add-on manifests, Orpheus language resources, RHVoice parameter files, SentencePiece package data, F2PY/Meson/NumPy build artifacts, Python pickle files, and selected add-on/cache metadata.</li>");
            html.AppendLine("<li>Added Apple Mach-O binary reporting for macOS dynamic libraries such as <code>.dylib</code>, including universal-binary architecture summaries and safe file-role fields without loading or executing code.</li>");
            html.AppendLine("<li>See <a href=\"#supported-file-types\">Supported file types</a> for the maintained coverage table.</li>");
            html.AppendLine("<li>Added clickable file links in multi-file report overviews. In the embedded HTML details view they select the target file in the current report; in exported HTML reports they jump to that file's section on the page.</li>");
            html.AppendLine("</ul>");
            ...
            html.AppendLine("<li>Added a scan-filtering workflow for large local folders and used it for a first Windows/program-folder coverage pass, adding Windows system resources, diagnostics, WMI/PowerShell support files, WinRT/XAML/resource files, font collections, device metadata, provisioning files, and common developer/script resources.</li>");
            html.AppendLine("<li>Added another uncommon-extension pass covering playlists, HTML Help contents files, old JAWS support files, speech assets, VLC skin catalogs, antivirus signatures, AppCache manifests, BeebEm/Torch MCP files, and Stylus RMX Korg controller scenes.</li>");
            ...
            html.AppendLine("<li>Added another candidate-list pass for common but easy-to-overlook support formats, including dictionaries, RTF, Jet/Access databases, COM type libraries, Windows Media/ASF containers, and Type 1 font support files.</li>");
