...
namespace FileDentify
{
    internal static partial class ManualService
    {
        ...
        private static string BuildHtml()
        {
            ...
            html.AppendLine("<p>FileDentify is a portable Windows file identification tool built for keyboard and screen-reader users. It combines the embedded LibFileDentify engine, Unix file/libmagic, format-specific analysis, readable text extraction, hashes, advanced viewing, reports, and optional companion tools.</p>");
        ...
        private static void GettingStarted(StringBuilder html)
        {
            ...
            html.AppendLine("<p>When a report contains more than one file, FileDentify adds a Report overview before the individual file reports. This is useful for spotting unknown files, inspection errors, large files, common strings, common byte patterns, and unusual type or extension clusters. In HTML details view, file names in overview lists can be opened to select that file in the current report. In exported HTML reports, those links jump to the file's section on the page.</p>");
        ...
        private static void SupportedFileTypes(StringBuilder html)
        {
            ...
            SupportedRow(html, "Windows system data", ".evtx, .etl, .wer, .dmp/.mdmp minidumps, .mum, .cdf-ms, .admx, .adml, .resw, .diagpkg, .compositefont, .efi, .pnf, .inf_loc, .gpd, .gdl, .mof, .mfl, .cdxml, .psd1, .psm1, .ps1xml, .xrm-ms, .sdb, catdb, .cat, .appx, .msix, .msixbundle, .manifest, .man, .msc, .mui, .mun, .winmd, .xbf, .pri, .nls, .ttc, .fon, .acm, .ax, .grxml, .wprp, .ppkg, .provxml, .devicemetadata-ms, .cip, .ppd, .regtrans-ms, .blf, .mo, .qm, .tlb, boot.sdi/.sdi, hosts, services, protocol, networks", "Event-log and trace-log headers, WER report fields, minidump header/stream counts and privacy notes, servicing package identity/version fields, component metadata markers, Group Policy and resource XML counts, diagnostic-package fields, composite-font roles, UEFI executable role and PE details, driver setup cache and localization clues, printer description directives, WMI and PowerShell structure counts, licensing XML counts, app compatibility database markers, catalog database markers, Windows app package manifest/signature presence, instrumentation manifest counts, MUI/MUN resource clues, WinRT/XAML/resource index hints, font collection counts, device/provisioning metadata roles, translation-catalog counts, COM type-library markers, boot SDI image hints, and network-config entries.");
            SupportedRow(html, "Certificates and signatures", ".cer, .crt, .der, .p7b, .p7c, AppX .p7x, .pfx, .p12, .pem", "X.509 certificate metadata, PKCS #7 and PKCX wrapper identity, PKCS #12/PFX sensitivity warnings, PEM block types and counts, and private-key warnings. FileDentify does not validate trust, install certificates, request passwords, or expose private-key payloads.");
            SupportedRow(html, "Documents, ebooks, subtitles, and data documents", ".pdf, .docx, .xlsx, .pptx, .odt, .ods, .odp, .rtf, .fodt, .epub, standalone .opf package documents, .smil, .srt, .acsm, .lit, .mobi, .prc, .pdb, .lrf, .brf, .cbr, .cbz, .chm, .hlp, .hhc, .fits", "Document metadata, ZIP/package structure, creator/date fields, EPUB package metadata/resource/reading-order counts, SMIL media-reference counts, SubRip cue/timing summaries, safe Adobe ebook-fulfillment metadata, embedded media/resource clues, RTF header fields and safe text preview, legacy ebook headers, Palm/MOBI database fields, braille text previews, comic archive hints, compiled help headers, HTML Help contents sidecars, and FITS scientific header cards.");
            SupportedRow(html, "Audio, media, images, and camera sidecars", ".flac, .mp3, extensionless ID3-tagged MP3, .mp2, .mpga, .ogg/.opus, .wav, .aiff, .mid, .au, .wma, .wmv, .asf, .m2ts, .ts, .mts, .mov, .mp4, .tga, .wmf, JPEG-based .thm camera thumbnails", "Header metadata, OpusHead/OpusTags fields, stream/container clues, raw MPEG frame estimates, ASF/Windows Media object markers, transport-stream packet details, ffprobe summaries when available, Targa dimensions/encoding, Windows Metafile geometry, and camera-thumbnail image/EXIF metadata without a false extension-mismatch warning.");
            ...
            SupportedRow(html, "Music projects and presets", ".als, .ablbundle, .abl, .ablpreset, .asd, Audacity .aup, Logic .mamd, Zoom LiveTrak .zdt, PMML music .pml, .rpp, .rpp-bak, .reaperthemezip, .ptx, WaveCache.wfm, .spn, .cpr, .npr, .all, .arr, .lso, .undo inside Logic packages, Logic Pro Library.bundle resources such as .cst/.acp/.aaz/.pst/.zxml/.ubs/.sdir_1, .sxt, .chtr, .nac, .nov, .h0, .hdp, .ovm, .akp, .akm, .rbm, .mdd, .fxp, .fxb, .vstpreset, .wrk, .cwp, .logikcs, .loopparam, .progindex, .jgl, .fxl, .fsl, Yamaha S-YXG .tbl files, .mod, .xm, .s3m, .it, .mogg, .syx, QWS .ini/.lng support files", "Project/preset identity, visible names, tracker details, SysEx clues, DAW-specific markers, Audacity project fields, Logic analysis-sidecar chunks, LiveTrak project version/channel labels, collision-safe PMML directives, Ableton analysis sidecars, Pro Tools session/cache sidecars, Logic key-command, library, patch, preset, and undo sidecars, REAPER project/theme packages, Reason NN-XT, GarageBand chord data, Native Instruments sample-add sidecars, MAGIX/SEK'D metadata, Akai sampler programs/multis, ReBirth mods, GN Audio embedded-MIDI containers, iKaossilator project/index sidecars, Roland Juno-G and Fantom Librarian data, Yamaha S-YXG table roles, and QWS instrument-map/transform/settings summaries.");
            SupportedRow(html, "Music software libraries", "Spitfire, XLN Audio, Spectrasonics .db/.db2/.ctl_rmx, Stylus RMX Korg controller scenes .k4s/.mks, and preset families, Korg, GForce M-Tron, Toontrack, Decent Sampler, Universal Audio LUNA, AIR Music Technology, Maize Sampler, Applied Acoustics Systems, Audio Modeling, UJAM, UJAM-style vendor blobs such as Crow Hill and Rhodes, Valhalla DSP, Modartt Pianoteq, Plugin Alliance .pabundle, Vital .vital/.vitalbank, Mac VST3 vendor resources including PACE Eden .winfo/.dsig, Sonic Charge .ivg/.ivgfont/.cushy/.makaron/.zlim, Universal Audio .tagset, iZotope .resembed/.path/.bt, Waldorf .vstxml/.gdr, and Native Instruments .nrc/.lua", "Product folder, library role, preset/bank names, controller scene markers, visible metadata, package sizes, VST3 bundle resource roles, and clear notes when proprietary payloads are not decoded.");
            SupportedRow(html, "Mac and Apple packages", ".logicx, .logic, .band, .garageband, Logic Pro Library.bundle, .sparsebundle, extensionless sparse-bundle bands, .DS_Store, iPhone/iPad backup folders, extensionless iOS backup payloads, .app, .framework, .bundle, .plugin, .appex, .xpc, .kext, .prefPane, .storyboardc, .momd, .rtfd, .component, .vst, .vst3, .clap, .aaxplugin, .ipa, .ipsw, .pkg, .car, .strings, .nib, .metallib, .sdef, .scpt, .kmmacros, .entitlements, .xcprivacy, .mobileconfig", "Package-folder identity, Info.plist fields, Finder folder metadata, project metadata, Logic library package structure, sparse-bundle details and qualified detached-band clues, mobile-backup manifests and hashed stored-file layout, app/archive structure, compiled UI/resource packages, compiled AppleScript identity, Keyboard Maestro macro structure/privacy guidance, scripting/privacy/entitlement files, and Apple resource hints.");
            SupportedRow(html, "Game, ROM, and game audio", ".nsf, .nes, .fds, .gb, .gbc, .gba, .nds, .sfc, .smc, .gen, .md, .sms, .gg, .n64, .z64, .vgm, .vgz, .sid, .s98, .rsn, .mini2sf, .wad, .chd, .pak, .vpk, .pk3, .pk4, .bsp, .mdl, .spr, .dem, .qwd, progs.dat/qwprogs.dat, .bpak, .bnk, .wem, .assets, .rpa, .acf, .vdf, .cue, .gdi, .sav, .srm, .ips, .bps, BBC Micro/BeebEm .ssd/.uef/PHROM and Torch MCP .abm/.cbl files, MoonShell/R4 .msp/.mse/.b15/.u8m/.glf/.l2u files, Nintendo Switch content folders, .nca.CONCAT segments", "ROM headers, mapper/platform clues, game package markers, VGM/SID/S98/RSN/2SF music fields, Quake map/demo/model/sprite/program clues, BBC Micro emulator-media and Torch MCP clues, Wwise/Unity/Ren'Py/Steam fields, MoonShell/R4 plugin/theme/support roles, Nintendo content-folder structure, and save/patch identity.");
            ...
            SupportedRow(html, "Firmware, mobile, and device files", ".sis, .sisx, Symbian .app/.aif/.rsc/.mbm/.mif/.mdl/.pdd/.kbd, Java ME .jad/.jar MIDlets, .imy, .mmf, .pmd, .qcp, .mld, .mxmf, .xmf, .rmf, .amr, .ota, .rtttl, .rtx, BIOS/UEFI images, Android boot images, U-Boot uImage, UF2, TRX, DTB, Intel HEX, Motorola S-record, Synology DSM/SRM .pat files by wrapper, Orbit Reader 20 Plus and ZOOM H/F recorder firmware .bin files, usb.ids/pci.ids, Roland SRX/SVD/SVQ/RFWV", "Installer/device headers, Symbian UID/resource/driver/keyboard-layout clues, MIDlet manifest fields, ringtone and old phone-audio fields, XMF/RMF music-container hints, firmware markers and embedded versions, ZOOM recorder model and verified official-package version, boot-image fields, router-firmware headers, Synology update-package wrapper clues, Orbit Reader firmware family and compatibility guidance, text firmware record summaries, hardware-ID database summaries, Roland identifiers, and visible package strings.");
            SupportedRow(html, "Personal data exports", ".contact, .vmg, .vcf, .ics, .opml, .eml, legacy Skype .dbb, .wme, .wpl, .m3u, .m3u8", "Windows Contact field counts, Nokia/Symbian message structure, vCard/contact counts, calendar/reminder counts, OPML subscription counts, email headers/MIME hints, privacy-preserving legacy Skype database roles, Windows Media Encoder session fields, Windows Media Player and M3U/M3U8 playlist entries, and privacy notes.");
            SupportedRow(html, "Mercury and Pegasus Mail", ".mer, .mlf, .cac, .pnc, .fff, .smp, .pmi, .pmj, .pmm, .pnm", "Mercury/32 rule, mailing-list, content-control, new-mail cache, and sample configuration roles, plus Pegasus/Mercury mailbox support sidecars and Pegasus form fact files. FileDentify reports bounded structure and counts without summarizing mailing-list members, message bodies, or address-bearing fields in the special section.");
            SupportedRow(html, "Browser and profile data", ".wdseml, .msf, .jsonlz4, Chromium Safe Browsing .store.* files, .sqlite/.sqlite3/.db/.db3, SQLite WAL/SHM sidecars", "Thunderbird message-search snippets, Mozilla mail indexes, Firefox/Thunderbird compressed profile JSON roles, Chromium Safe Browsing store names, SQLite database roles, visible schema strings, and privacy notes for shareable reports.");
            SupportedRow(html, "Programming and app resources", ".apk, .nvda-addon, installed NVDA manifest.ini files, Java .jmod, Lua .luac, Wavefront .mtl, .asar, .nupkg, .deb, .crx3, .fbx, .mgc, Tcl .enc maps, .pyc, .wasm, .pak, .msg, .plist, .json, .sqlite/.sqlite3/.db/.db3, .fdb, .mdb, .ico, .eot, .icc/.icm, .pfb, .pfm, fonts, .dic, .aff, C/C++/SDK source and resource files such as .h/.c/.cpp/.idl/.rc, .rst, .cmake, Meson build files, .zig, .pyi, .pyf, .f2py_f2cmap, .pyx/.pxd/.pxi, .npz, .pkl/.pickle, .pot, .g4, .pth, .properties, .ui, .bas, .nsi/.nsh, .sh, .f/.f90, .glsl/.hlsl/.fx, .dtd, .yang, .po, .afm, .lib, .obj, .a, .map, .vim, .pm, .pl, .tcl, .lua, .nse, .adoc, .props, .targets, .resx, .xsd, .xsl/.xslt, .qml, .qmltypes, .bcmap, .cjs, .mjs, .node, .pyd, .natvis, .ide antivirus signatures, .appcache manifests, VLC .catalog files, MAXON/Cinema 4D .str/.c4dnodes resources, NVIDIA .nvi/.nvx/.nifx/.forms resources, Chocolatey .arguments/.files/.ignore support files, MAGIX/Sound Forge .mxmd/.mxdh/.mxdk/.mxdp/.sfexp resources, Chromium .hyb dictionaries, LibreOffice/OpenOffice .soc/.sod/.soe/.sog/.soh/.sor resources, liblouis .ctb/.utb/.uti/.cti/.dis/.tbl tables, legacy JAWS .jnf/.jrf files, Rockbox .rock plug-ins, MilkDrop .milk presets, Winamp .maki scripts and .w5s modules, Microsoft Chat .avb avatars, SentencePiece package tables, Hugging Face download metadata, NAO document-cache metadata, IANA timezone tables, Nokia ringtone demos, Piper/Sonata/SuperTonic .onnx voice models, Google TTS for NVDA .zvoice packages and voices.json catalogs, Microsoft voice .msix/.bin/.dat/.ini/.xml/.txt files, AT&amp;T/IVONA/Loquendo voice packages, ModelTalker .cbk files, Dolphin Orpheus, application-map, default keyboard/language, and Nuance Vocalizer Expressive speech data, Eloquence/IBM TTS .syn modules, RHVoice voice.data/.fst/.params/model files, FlexVoice .bin/.dat files, eSpeak NG dictionaries/phoneme data, Acapela .qvcu/.nuul216/.clb voice data", "Bytecode/module/resource headers, Java module-archive and Lua bytecode versions, Wavefront material names and texture references, Android and NVDA add-on package structure, Electron ASAR indexes, NuGet/Debian/Chrome package hints, FBX model headers, magic.mgc identity, Tcl encoding maps, SQLite/Firebird/plist/Jet database details, Windows icon and web-font entries, ICC color-profile fields, Type 1 font support strings, dictionary and affix-rule summaries, developer source/script/resource role clues, safe line and declaration counts, native extension module identity without loading, vendor resource hints, browser and office support-resource roles, accessibility table metadata, legacy application resource roles, speech-voice metadata, antivirus identity/update clues, app-cache manifest roles, and font/container signatures.");
            ...
            SupportedRow(html, "AI model files", "Ollama manifests, Ollama blobs, GGUF, TensorFlow Lite .tflite, ONNX Runtime .ort, PyTorch .ckpt/.pt/.pth checkpoints, SafeTensors .safetensors", "Model tag, manifest media type, referenced layers, GGUF version, tensor count, metadata count, TensorFlow Lite and ONNX Runtime model roles, PyTorch archive or pickle container hints, SafeTensors header size, and visible metadata keys.");
        ...
        private static void ReportSections(StringBuilder html)
        {
            ...
            Row(html, "Safety hints", "A concise high-confidence content and filename mismatch summary, an active-content risk only where applicable, and practical advice to verify the source and use trusted security scanning. FileDentify reports structure and visible metadata; it does not certify that a file is harmless.");
            ...
            Row(html, "Speech voice", "Piper/Sonata, SuperTonic, Pocket TTS, DoubleTalk PC/DoubleTalk64, Telesensory Systems Prose 2000, Google TTS for NVDA, Microsoft natural voice/SAPI, AT&amp;T Natural Voices, IVONA, Loquendo, Dolphin Orpheus, Dolphin application/default data, Nuance Vocalizer Expressive, Eloquence/IBM TTS, RHVoice, FlexVoice, eSpeak NG, STSpeech, BestSpeech, tgSpeechBox, and Acapela voice files, including ONNX model role, voice folder, JSON voice configuration fields such as sample rate, quality, eSpeak voice, language, speaker counts where visible, NumPy voice-embedding shape and dtype, Pocket TTS tokenizer/model roles, DoubleTalk ROM identity/version and pronunciation resources, exact-fingerprint Prose 2000 ROM role/version/socket details, Google .zvoice package contents and local catalog speakers, Microsoft package/index/model roles and payload lists, voice archive payload sizes where ZIP indexes are readable, speech engine/add-on manifest details, Dolphin Orpheus .tts/.phm/.vcx language resources, Dolphin voice preference tables and sidecars, RHVoice HTS fields and parameter files, eSpeak dictionary/phoneme roles, and Acapela voice-data role and size.");
            ...
            Row(html, "Database", "SQLite-family databases such as .sqlite, .sqlite3, .db, and .db3, Firebird .fdb files, plus Microsoft Access/Jet database headers, version markers, visible schema strings, and visible object/table names where present. FileDentify does not open tables, run queries, or extract records.");
            Row(html, "Scientific data", "FITS scientific data files, including header-card counts and common cards such as BITPIX and NAXIS. FileDentify does not render images or parse data tables.");
            Row(html, "Color profile", "ICC/ICM color profiles, including device class, color space, PCS, and acsp marker. FileDentify does not apply or validate color transforms.");
            ...
            Row(html, "Developer/app resources", "Application support resources such as Electron ASAR archives, NuGet packages, source/script files, MSBuild/XML resources, QML, source maps, PDF.js CMaps, Node/Python native modules, Chromium hyphenation .hyb dictionaries, LibreOffice/OpenOffice .soc/.sod/.soe/.sog/.soh/.sor resources, MAXON/Cinema 4D resources, NVIDIA installer manifests, Chocolatey support files, MAGIX/Sound Forge resources, and Winamp .w5s modules, including role clues and bounded metadata without executing or loading code.");
            ...
            Row(html, "Logic Pro library", "Logic Pro Library.bundle package folders and internal user-library resources, including .cst channel strip settings, .acp Alchemy presets, .aaz Alchemy sample payloads, .pst plug-in settings, .zxml patch cache XML, .ubs Ultrabeat samples, .sdir_1 Space Designer sidecars, LibraryUUID, and bounded folder summaries.");
            ...
            Row(html, "VST3 vendor resources", "Internal resources from Mac VST3 bundles, including PACE Eden .winfo/.dsig wrapper metadata, Sonic Charge .ivg/.ivgfont/.cushy/.makaron/.zlim UI/resource files, Universal Audio .tagset files, iZotope .resembed/.path/.bt resources, Waldorf .vstxml/.gdr metadata, and Native Instruments VST3 .nrc resource containers and controller Lua scripts. FileDentify reports role, bundle, visible metadata, and safety notes without loading plug-in code.");
            Row(html, "Apple Mach-O binary", "macOS and iOS Mach-O binaries such as .dylib files, including universal/fat-binary architecture summaries, CPU families, file role, containing bundle where visible, and a clear no-load/no-execute note.");
            Row(html, "Audio sample resource", "Arturia .arta/.astr/.artx/.afx/.mnfx/.ag*/.ae*/.fct/.fct2/.seq/.raw12b/.sfzi/.sfzh/.eiiwav resources, ROLI .roliaudio, Initial Audio .ignitex, Guitar Rig .grir, Apple .caf/.sdir, Scala .scl, and wavetable .wt files, including product/vendor context, library folder, role, CAF chunk markers, scale text, visible strings, XML preset fields, MiniFreak serialization clues, and proprietary-payload notes.");
            ...
            Row(html, "Roland Cloud", "Roland Cloud expansion and preset files such as .exz, .sdz, VEXP headers, KoaBankFile preset banks, Preset.bin, Initial.bin, TextCodeTable.dat, D-50 waveform ROM data inside VST3 bundles, and InstalledBankNames.dat, including product folder, expansion code/name, visible bank name, table strings, and installed bank strings where available.");
            ...
            Row(html, "Korg", "Korg sample-library, workstation, and KORG Collection data, including WaveMotion .wmss files, Mono/Poly .mp4prog presets, Electribe-R .er1 patterns, KORG Collection .program/XML presets, VST .fxp/.fxb banks, Trinity/Triton .pcg, .ksc, .kmp, RAWSND, PCM .bin resources, split high/low PCM chunks, controller maps, preset/index JSON, extensionless Korg objects, ADSR, voice amp, pitch, arpeggiator, vector-envelope, database, product folder, role, visible object markers, preset names, script references, and sample-map references where available.");
            ...
            Row(html, "UJAM", "UJAM content blobs, preset patches, NKS files, settings, and metadata, including content-based part-one index recognition, instrument names, content UUID, format version, product folder, role, file size, preset names, build metadata, and DSP setting counts where visible.");
            Row(html, "UJAM-style blob", "UJAM-style .blob payloads used by UJAM and other sample-library vendors such as Crow Hill and Rhodes, including conservative detached part-two recognition, vendor/product context when available, file size, leading UUID, and a proprietary-payload note.");
            ...
            Row(html, "Steinberg Cubase and VST", "Cubase, Nuendo, VST preset/project extensions, and Windows VST plug-in SDK marker strings. Project and preset files can include readable clues such as ASIO/VST markers, MIDI edit commands, drum-map names, effects, grooves, fonts, internal markers, and VST FXP/FXB header fields where present; plug-in DLLs are reported as visible VST markers only and are not loaded.");
            ...
            Row(html, "Developer/app resources", "Electron ASAR archives, NuGet packages, source/script files, MSBuild/XML resources, QML resources, source maps, PDF.js CMaps, native Node/Python extension modules, vendor resource manifests, and app support files, including ASAR header JSON size, top-level entries, file/directory counts, package.json presence, NuGet nuspec metadata, safe line/declaration counts, product/path hints, and native-module identity without loading executable code.");
            Row(html, "Java module, Lua bytecode, and 3D material library", "JMOD header version and intended contents, Lua binary-chunk version fields, and Wavefront MTL material names and texture references. FileDentify does not execute bytecode or load model resources.");
            Row(html, "EPUB package document", "Metadata and package structure from standalone .opf files, including EPUB package version, title, creator, language, identifier, manifest count, and reading-order count.");
            Row(html, "Certificate / signature data", "Certificate and signature-container identity, safe X.509 public metadata, PEM block types, and clear warnings for PFX or private-key content. FileDentify does not import certificates, request passwords, validate trust, or expose private-key payloads.");
            Row(html, "New structured and legacy formats", "Windows Contact XML, SMIL synchronized multimedia, SubRip subtitles, ACSM ebook fulfillment tokens, Audacity AUP projects, Logic MAMD sidecars, Zoom LiveTrak project data, Keyboard Maestro macro libraries, compiled AppleScripts, legacy Skype databases, Symbian drivers and keyboard layouts, PMML music source, Targa and Windows Metafile images, and camera THM thumbnails. Collision-prone extensions require expected content markers, and sensitive formats include privacy guidance.");
            ...
            Row(html, "Firmware / device image", "PC BIOS/UEFI firmware, Android boot images, U-Boot legacy uImage files, UF2 microcontroller firmware, Broadcom/OpenWrt TRX router firmware, flattened device tree blobs, Intel HEX, Motorola S-records, Synology DSM/SRM PAT update packages, Orbit Reader 20 Plus firmware, ZOOM H-series and F-series recorder firmware, Roland expansion data, file size, visible firmware strings, recorder model, verified official-package version, header fields, compatibility guidance, record summaries, and a clear no-flash/no-modify note.");
            ...
            Row(html, "Mercury/Pegasus mail", "Mercury/32 and Pegasus Mail support files such as rules, mailing lists, mailbox cache/index/state sidecars, content-control sets, form fact files, and sample configuration files. FileDentify reports roles, counts, and safe structure clues while avoiding address/member/body summaries in this special section.");
            Row(html, "Windows/system data", "Windows system and application-support files such as EVTX/ETL logs, WER reports, servicing MUM manifests, CDF-MS component metadata, ADMX/ADML Group Policy templates, RESW resources, diagnostic package XML, composite fonts, PNF/INF localization data, GPD/GDL/PPD printer descriptions, MOF/MFL WMI files, PowerShell CDXML/PSD1/PSM1/PS1XML support files, XRM-MS licensing XML, SDB compatibility databases, catalog databases, AppX/MSIX packages, MUI/MUN resource modules, WinMD metadata, XBF/Pri resources, NLS data, TrueType collections, ACM/DirectShow modules, speech grammar XML, provisioning/device metadata, Code Integrity policies, registry transaction logs, legacy .fon bitmap font libraries, gettext/Qt translation catalogs, boot SDI ramdisk images, network config files, instrumentation manifests, and application manifests. These are reported from headers, filenames, and safe text/XML/package structure only.");
        ...
        private static void Credits(StringBuilder html)
        {
            ...
            html.AppendLine("<ul>");
            Credit(html, "LibFileDentify", "https://github.com/OnjLouis/LibFileDentify", "", "", "the reusable evidence-based identification engine developed alongside FileDentify");
        ...
        private static void License(StringBuilder html)
        {
            ...
            html.AppendLine("<p>FileDentify and its embedded LibFileDentify engine are released under the MIT License. Bundled third-party components have their own notices under Help &gt; Third-party notices.</p>");
        }
