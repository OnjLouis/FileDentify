...
namespace FileDentify
{
    internal static class ManualService
    {
        ...
        private static void CommandLineOptions(StringBuilder html)
        {
            ...
            Row(html, "FileDentify.exe --report report.txt [files...]", "Write a plain text report and exit. The file list can contain multiple non-contiguous files from different folders or drives.");
            Row(html, "FileDentify.exe --report report.html [files...]", "Write an HTML report and exit. The file list can contain multiple non-contiguous files from different folders or drives.");
            Row(html, "FileDentify.exe -r report.txt [files...]", "Short form of --report.");
            Row(html, "FileDentify.exe --html-report report.html [files...]", "Explicitly write an HTML report and exit.");
            Row(html, "FileDentify.exe --folder-report report.txt [folders-or-files...]", "Recursively scan folders and write one complete plain-text report.");
            Row(html, "FileDentify.exe --folder-report report.html [folders-or-files...]", "Recursively scan folders and write one complete HTML report.");
            Row(html, "FileDentify.exe --viewer-output output.txt --viewer-mode hex file", "Write advanced viewer output without opening the graphical interface.");
            Row(html, "FileDentify.exe --viewer --viewer-mode readable file", "Write advanced viewer output to standard output.");
            Row(html, "FileDentify.exe --viewer-bytes 4194304", "Limit command-line advanced viewer output to this many bytes per file.");
            ...
            Row(html, "fd.com -u", "Check GitHub Releases for updates from a terminal.");
            Row(html, "FileDentify.exe --close", "Ask other FileDentify windows from the same executable to close gracefully.");
            Row(html, "FileDentify.exe -u", "Check GitHub Releases for updates.");
            Row(html, "FileDentify.exe --update", "Long form of update check.");
            Row(html, "FileDentify.exe --install-sendto", "Add FileDentify to the Windows Send To menu.");
            Row(html, "FileDentify.exe --uninstall-sendto", "Remove FileDentify from the Windows Send To menu.");
            Row(html, "FileDentify.exe --install-desktop", "Create a FileDentify shortcut on the desktop.");
            Row(html, "FileDentify.exe --uninstall-desktop", "Remove the FileDentify desktop shortcut.");
            Row(html, "FileDentify.exe --version", "Show the current FileDentify version.");
            Row(html, "FileDentify.exe --help", "Show command-line help.");
            Row(html, "FileDentify.exe -h", "Short form of --help.");
            html.AppendLine("</tbody></table>");
