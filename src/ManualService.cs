...
namespace FileDentify
{
    internal static class ManualService
    {
        ...
        private static void SupportedFileTypes(StringBuilder html)
        {
            ...
            SupportedRow(html, "Windows system data", ".evtx, .pnf, .inf_loc, .gpd, .gdl, .xrm-ms, .sdb, catdb, .cat, .appx, .msix, .msixbundle, .manifest, .man, .mo, .qm, boot.sdi/.sdi, hosts, services, protocol, networks", "Event-log headers, driver setup cache and localization clues, printer description directives, licensing XML counts, app compatibility database markers, catalog database markers, Windows app package manifest/signature presence, instrumentation manifest counts, translation-catalog counts, boot SDI image hints, and network-config entries.");
            SupportedRow(html, "Documents and ebooks", ".pdf, .docx, .xlsx, .pptx, .odt, .ods, .odp, .epub, .lit, .mobi, .prc, .pdb, .lrf, .brf, .cbr, .cbz, .chm, .hlp", "Document metadata, ZIP/package structure, creator/date fields, counts, embedded media/resource clues, legacy ebook headers, Palm/MOBI database fields, braille text previews, comic archive hints, and compiled help headers.");
            ...
            SupportedRow(html, "Sample and instrument banks", ".sf2, .sbk, .sfArk, .dls, .ecw, .sfz, .exs, .ufs, .blob, .jlw, .vop, .scl, Ensoniq .efe/.eda/.edt", "Bank metadata, visible preset/instrument/sample names, microtuning scale text, container hints, legacy sound-bank clues, Ensoniq EPS/ASR/TS header/catalog strings, and proprietary payload notes.");
            ...
            SupportedRow(html, "Music projects and presets", ".als, .ablbundle, .abl, .ablpreset, .rpp, .rpp-bak, .ptx, WaveCache.wfm, .spn, .cpr, .npr, .all, .arr, .fxp, .fxb, .vstpreset, .wrk, .cwp, .logikcs, .loopparam, .progindex, .jgl, .mod, .xm, .s3m, .it, .mogg, .syx, QWS .ini/.lng support files", "Project/preset identity, visible names, tracker details, SysEx clues, DAW-specific markers, Pro Tools session/cache sidecars, Logic key-command exports, iKaossilator project/index sidecars, Roland Juno-G Librarian data, and QWS instrument-map/transform/settings summaries.");
            ...
            SupportedRow(html, "Mac and Apple packages", ".logicx, .logic, .band, .garageband, .sparsebundle, iPhone/iPad backup folders, extensionless iOS backup payloads, .app, .framework, .bundle, .plugin, .appex, .xpc, .kext, .prefPane, .storyboardc, .momd, .rtfd, .component, .vst, .vst3, .clap, .aaxplugin, .ipa, .ipsw, .pkg, .car, .strings, .nib, .metallib, .sdef, .entitlements, .xcprivacy, .mobileconfig", "Package-folder identity, Info.plist fields, project metadata, sparse-bundle details, mobile-backup manifests and hashed stored-file layout, app/archive structure, compiled UI/resource packages, scripting/privacy/entitlement files, and Apple resource hints.");
            SupportedRow(html, "Game, ROM, and game audio", ".nsf, .nes, .fds, .gb, .gbc, .gba, .nds, .sfc, .smc, .gen, .md, .sms, .gg, .n64, .z64, .vgm, .vgz, .sid, .s98, .rsn, .mini2sf, .wad, .chd, .pak, .vpk, .pk3, .pk4, .bsp, .mdl, .spr, .dem, .qwd, progs.dat/qwprogs.dat, .bpak, .bnk, .wem, .assets, .rpa, .acf, .vdf, .cue, .gdi, .sav, .srm, .ips, .bps, MoonShell/R4 .msp/.mse/.b15/.u8m/.glf/.l2u files, Nintendo Switch content folders, .nca.CONCAT segments", "ROM headers, mapper/platform clues, game package markers, VGM/SID/S98/RSN/2SF music fields, Quake map/demo/model/sprite/program clues, Wwise/Unity/Ren'Py/Steam fields, MoonShell/R4 plugin/theme/support roles, Nintendo content-folder structure, and save/patch identity.");
            ...
            SupportedRow(html, "Firmware, mobile, and device files", ".sis, .sisx, Symbian .app/.aif/.rsc/.mbm/.mif/.mdl, Java ME .jad/.jar MIDlets, .imy, .mmf, .pmd, .qcp, .mld, .mxmf, .xmf, .rmf, .amr, .ota, .rtttl, .rtx, BIOS/UEFI images, Android boot images, U-Boot uImage, UF2, TRX, DTB, Intel HEX, Motorola S-record, usb.ids/pci.ids, Roland SRX/SVD/SVQ/RFWV", "Installer/device headers, Symbian UID/resource clues, MIDlet manifest fields, ringtone and old phone-audio fields, XMF/RMF music-container hints, firmware markers, boot-image fields, router-firmware headers, text firmware record summaries, hardware-ID database summaries, Roland identifiers, and visible package strings.");
            SupportedRow(html, "Personal data exports", ".vmg, .vcf, .ics, .opml, .eml, .wme", "Nokia/Symbian message structure, vCard/contact counts, calendar/reminder counts, OPML subscription counts, email headers/MIME hints, Windows Media Encoder session fields, and privacy notes.");
            SupportedRow(html, "Programming and app resources", ".apk, .nvda-addon, .asar, .nupkg, .pyc, .wasm, .pak, .msg, .plist, .json, .sqlite, fonts, liblouis .ctb/.utb/.uti/.cti/.dis tables, Rockbox .rock plug-ins, MilkDrop .milk presets, Winamp .maki scripts, Microsoft Chat .avb avatars, Piper/Sonata/SuperTonic .onnx voice models, Microsoft voice .msix/.bin/.dat/.ini/.xml/.txt files, AT&amp;T/IVONA/Loquendo voice packages, Dolphin Orpheus, application-map, default keyboard/language, and Nuance Vocalizer Expressive speech data, Eloquence/IBM TTS .syn modules, RHVoice voice.data/.fst/model files, FlexVoice .bin/.dat files, eSpeak NG dictionaries/phoneme data, Acapela .qvcu/.nuul216 voice data", "Bytecode/module/resource headers, Android and NVDA add-on package structure, Electron ASAR indexes, NuGet nuspec metadata, SQLite/plist details, accessibility table metadata, legacy application resource roles, speech-voice metadata, and font/container signatures.");
        ...
        private static void ReportSections(StringBuilder html)
        {
            ...
            Row(html, "Accessibility data", "liblouis braille translation, display, contraction, and include tables such as .ctb, .utb, .uti, .cti, and .dis files, including table title, included tables, opcode-line counts, and a note that FileDentify does not compile or validate the table.");
            ...
            Row(html, "Game/ROM data", "Header-level details for game and emulator files such as NSF, iNES/NES, Game Boy, GBA, Nintendo DS, SNES, Genesis/Mega Drive, Sega Master System/Game Gear, Nintendo 64, Doom WAD, CHD, Quake PACK/BSP/demo/model/sprite/progs.dat, protected .bpak packages, Wwise BNK/WEM, Unity assets, Ren'Py archives, Steam ACF/VDF, generic .rom files, game saves, disc layout files, and common patch/package extensions.");
            Row(html, "Legacy music/game audio", "Older game, mobile, MIDI, and audio-production formats such as VGM/VGZ, SID, S98, RSN, mini2SF, QCP, XMF/MXMF, MFi/MLD, Beatnik RMF, Korg QSEQ, Recomposer RCP/HED/G36, WRD/LYC/MAG/ZEL/GMC sidecars, Cubase/Sound Forge waveform overview files, microtuning .tun maps, SFI/SFIP impulse data, and raw .sam samples.");
            ...
            Row(html, "Firmware / device image", "PC BIOS/UEFI firmware, Android boot images, U-Boot legacy uImage files, UF2 microcontroller firmware, Broadcom/OpenWrt TRX router firmware, flattened device tree blobs, Intel HEX, Motorola S-records, Roland expansion data, file size, visible firmware strings, header fields, record summaries, and a clear no-flash/no-modify note.");
            ...
            Row(html, "Message/contact data", "Personal/export formats such as Nokia/Symbian VMG messages, vCard contacts, iCalendar calendars/reminders, OPML subscription lists, EML email messages, and Windows Media Encoder sessions. These sections summarize structure and key metadata and include privacy notes because the files often contain personal addresses, phone numbers, locations, message bodies, URLs, or device paths.");
            Row(html, "Windows/system data", "Windows system and application-support files such as EVTX logs, PNF/INF localization data, GPD/GDL printer descriptions, XRM-MS licensing XML, SDB compatibility databases, catalog databases, AppX/MSIX packages, gettext/Qt translation catalogs, boot SDI ramdisk images, network config files, instrumentation manifests, and application manifests. These are reported from headers, filenames, and safe text/XML/package structure only.");
